...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 z-50 w-full border-b transition-all duration-300 ${
      scrolled
        ? "border-gray-200 bg-white/98 shadow-lg shadow-black/5 backdrop-blur-md"
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3.5 sm:px-6 sm:py-4 lg:px-10">
        ...
        >
          ...
          <span className="truncate text-xl font-bold uppercase text-emerald-600 sm:text-2xl lg:text-3xl">
            Herfa
          </span>
        </motion.div>
        ...
        <div className="hidden lg:flex items-center gap-10 text-[13px] font-bold uppercase tracking-tight text-slate-500">
          <motion.button
            ...
            className="text-[15px] hover:text-emerald-500 transition-colors relative group"
            whileHover={{ y: -1 }}
          >
            Services
          ...
          <Link
            href="/about"
            className="transition-colors hover:text-emerald-500 relative group"
          >
            How it Works
            <span className="absolute -bottom-1 left-0 h-0.5 w-0 bg-emerald-500 transition-all duration-300 group-hover:w-full" />
          </Link>
          ...
          {!isAuthenticated ? (
            <>
              ...
              <motion.button
                ...
                className="rounded-full bg-emerald-600 px-5 py-2.5 text-sm text-white shadow-lg shadow-emerald-500/20 transition-all hover:bg-emerald-700 lg:px-8 lg:text-[15px]"
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <motion.span
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute -right-0.5 -top-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-bold text-white shadow-sm"
                      >
                ...
                <AnimatePresence>
                  {showDropdown && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      transition={{ duration: 0.2 }}
                      className="absolute right-0 mt-4 flex w-60 translate-y-1 flex-col gap-1 rounded-3xl border border-gray-100 bg-white p-2 shadow-2xl"
                    >
                      <button
                      ...
                      {user?.role === "artisan" && (
                        <>
                          <button
                            onClick={() => handleNavigate("/artisan/create-profile")}
                            className="flex w-full items-center gap-4 rounded-2xl p-4 text-sm font-bold text-slate-700 transition-colors hover:bg-emerald-50"
                          >
                          ...
                          <button
                            onClick={() => handleNavigate("/artisan/edit-profile")}
                            className="flex w-full items-center gap-4 rounded-2xl p-4 text-sm font-bold text-slate-700 transition-colors hover:bg-emerald-50"
                          >
                            <Settings size={18} className="text-emerald-500" />
      ...
      <AnimatePresence>
        {showMobileMenu && (
          <motion.div
            ...
            className="border-t border-slate-100 bg-white overflow-hidden sm:hidden"
          >
            <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-4">
              <motion.button
                onClick={handleServicesClick}
                className="rounded-2xl bg-slate-50 px-4 py-3.5 text-left text-sm font-semibold text-slate-700 active:bg-slate-100"
              ...
              {!isAuthenticated ? (
                <>
                  <motion.button
                    onClick={() => handleNavigate("/auth/login")}
                    className="rounded-2xl border border-slate-200 px-4 py-3.5 text-left text-sm font-semibold text-slate-700 active:bg-slate-50"
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                  ...
                  <motion.button
                    onClick={() => handleNavigate("/auth/signup")}
                    className="rounded-2xl bg-emerald-600 px-4 py-3.5 text-left text-sm font-semibold text-white active:bg-emerald-700"
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                ...
                <>
                  ...
                  <motion.button
                    onClick={handleLogout}
                    className="rounded-2xl bg-red-50 px-4 py-3.5 text-left text-sm font-semibold text-red-500 active:bg-red-100"
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
